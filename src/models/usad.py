...
from src.models.usad_utils import USADModel, fit_with_early_stopping, predict_test_scores


class USAD(Algorithm, PyTorchUtils):
    """USAD Algorithm.
    """
    def __init__(self, name: str='usad', num_epochs: int=100, batch_size: int=128,
                learning_rate: float=1e-3, hidden_size: int=5, window_size: int=10,
                train_val_percentage: float=0.25, verbose=True, seed: int=None,
                alpha: float=.5, beta: float=.5, gpu: int=None, patience: int=20,
                save_dir='data', multi_outputs=True):
        """USAD algorithm for anomaly detection.
        ...
        Args:
            ...
            window_size (int, optional)             : The size of the moving window. 
                                                        Defaults to 10.
            alpha (float, optional)                 : USAD trade-off parameters.
                                                        Defaults to 0.5.
            beta (float, optional)                  : USAD trade-off parameters.
                                                        Defaults to 0.5.
    ...
    def compute_additional_params(self, whole_data_loader):
        ...
        with torch.no_grad():
            for ts_batch in whole_data_loader:
                ...
                outs1 = self.model.decoder_1(self.model.encoder(ts_batch))
                outs2 = self.model.decoder_2(self.model.encoder(outs1))

                score1, score2 = self.model.compute_test_score(ts_batch, outs1, outs2)
                score = self.alpha*score1 + self.beta*score2
                reconstr_errors.append(score.cpu().numpy().tolist())
        #print(reconstr_errors[:10])
        ...
        if len(reconstr_errors) > 0:
            ...
            #reconstr_errors = np.concatenate(reconstr_errors, axis=0)
    ...
    def fit(self, train_data : np.array, categorical_columns=None):
        ...
        self.model = USADModel(train_data.shape[-1], self.hidden_size, seed=self.seed, gpu=self.gpu)
    ...
    def predict(self, test_data : np.array):
        ...
        test_loader = DataLoader(dataset=test_data, batch_size=self.batch_size,
                                ...
                                shuffle=False)
        reconstr_errors = predict_test_scores(self.model, test_loader, alpha=self.alpha,
                                            beta=self.beta)
