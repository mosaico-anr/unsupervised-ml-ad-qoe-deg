...
class USAD(Algorithm, PyTorchUtils):
    ...
    def compute_additional_params(self, whole_data_loader):
        ...
        with torch.no_grad():
            for ts_batch in whole_data_loader:
                ...
                w1 = self.model.decoder_1(self.model.encoder(ts_batch))
                w2 = self.model.decoder_2(self.model.encoder(w1))

                score1, score2 = self.model.compute_test_score(ts_batch, w1, w2)
    ...
    def predict(self, test_data : np.array):
        ...
        test_loader = DataLoader(dataset=test_data, batch_size=self.batch_size, drop_last=False, pin_memory=True,
                                 shuffle=False)
        reconstr_errors = predict_test_scores(self.model, test_loader, alpha=self.alpha, beta=self.beta)
