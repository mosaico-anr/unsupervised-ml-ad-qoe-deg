...
class DAGMM(Algorithm, PyTorchUtils):
    ...
    def compute_additional_parameters(self, whole_data_loader):
        ...
        with torch.no_grad():
            for ts_batch in whole_data_loader:
                ...
                # Forward pass
                code, x_prime, _, z_r, gamma_hat = self.model(ts_batch)
                phi, mu, cov_mat = self.model.compute_params(z_r, gamma_hat)
                energy_result, pen_cov_mat = self.model.estimate_sample_energy(z_r, phi, mu, cov_mat)
            ...
            for ts_batch in whole_data_loader:
                ...
                code, x_prime, _, z_r, gamma_hat = self.model(ts_batch)
                phi, mu, cov_mat = self.model.compute_params(z_r, gamma_hat)
                sample_energy, cov_diag = self.model.estimate_sample_energy(z_r, phi=train_phi, mu=train_mu, cov_mat=train_cov,
    ...
    def fit(self, train_data : np.array, categorical_columns=None):
        ...
        self.model = DAGMMModel(input_length=train_data.shape[-1], hidden_size=self.hidden_size,
                                lambda_1=self.lambda_1, lambda_2=self.lambda_2, reg_covar=self.reg_covar,
    ...
    def predict(self, test_data : np.array):
        ...
        test_loader = DataLoader(dataset=test_data, batch_size=self.batch_size, drop_last=False, pin_memory=True,
                                 shuffle=False)
        test_energy, _ = predict_test_scores(self.model, test_loader, train_phi=self.additional_params['train_phi'], 
