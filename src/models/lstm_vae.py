...
from src.models.lstm_vae_utils import LSTMVAE, fit_with_early_stopping, predict_test_scores



class LSTMVAEAlgo(Algorithm, PyTorchUtils):
    """LSTM VAE Algorithm."""
    def __init__(self, name: str='lstm_vae', num_epochs: int=100, batch_size: int=128,
                learning_rate: float=1e-3, lstm_dim : int=15, hidden_size: int=3,
                window_size: int=10, train_val_percentage: float=0.25, verbose=True,
                seed: int=None, gpu: int=None, patience: int=20, save_dir='data',
                multi_outputs=True):
    ...
    def anomaly_vector_construction(self, test_recons_errors, sigma=3):
    ...
    def fit(self, train_data : np.array, categorical_columns=None):
        ...
        self.model = LSTMVAE(n_features=train_data.shape[1], hidden_size=self.hidden_size,
