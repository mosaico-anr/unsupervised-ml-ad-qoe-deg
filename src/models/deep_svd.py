...
class DeepSVDD(Algorithm, PyTorchUtils):
    ...
    def __init__(self, name: str='deep_svdd', num_epochs: int=100, batch_size: int=128,
                center = None, radius: float=0.0, objective: float= 'soft',
                learning_rate: float=1e-3, window_size: int=10,
        ...
        Args:
            ...
            center (torch.Tensor, optional)         : Hypersphere center.
                                                        Default to None.
            radius (float, optional)                : Hypersphere radius.
                                                        Defaults to 0.0.
    ...
    def fit(self, train_data : np.array, categorical_columns=None):
        ...
        self.model = DeepSVDDModel(diameter=train_data.shape[-1], seed=self.seed, gpu=self.gpu)
        print(f"Fitting {self.name} model")
        writer = SummaryWriter(self.save_dir)
        self.model, self.center, self.radius = \
                fit_with_early_stopping(train_loader,
                                        val_loader,
                                        self.model,
                                        patience=self.patience,
                                        num_epochs=self.num_epochs,
                                        learning_rate=self.learning_rate,
                                        writer=writer, center=self.center,
                                        radius=self.radius,
                                        objective=self.objective,
                                        verbose=self.verbose)
        print("Fitting done")
        self.init_params['center'] = self.center
        self.init_params['R'] = self.radius
